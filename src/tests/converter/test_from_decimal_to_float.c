...
START_TEST(test) {
  for (unsigned long i = 0; i < sizeof(src) / sizeof(s21_decimal); ++i) {
    float tmp;
...
START_TEST(s21_invalid_result) {
  ...
  int ret = s21_from_decimal_to_float(invalid, NULL);
...
Suite *test_from_decimal_to_float(void) {
  ...
  s = suite_create("\033[47m==S21_FROM_DECIMAL_TO_FLOAT==\033[0m");
